...
Uses HTTP API for search. Requires project root config when run from repo.
"""
...
class QdrantRagRepository:
    ...
    def search(
        ...
        try:
            ...
            raise RetrievalError(f"Qdrant search error (collection={coll}): {e.response.text}") from e
