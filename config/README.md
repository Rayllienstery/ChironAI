...
| Extensions registry | `server.yaml` (`extensions.registry_url`, `extensions.local_fallback_url`, `extensions.blocklist_url`) | `CHIRONAI_EXTENSIONS_REGISTRY_URL`, `CHIRONAI_EXTENSIONS_LOCAL_REGISTRY_FALLBACK`, `CHIRONAI_EXTENSIONS_BLOCKLIST_URL` | Defaults to the GitHub-hosted `ChironAI-Extensions-Registry` with local fallback to `extensions/registry/extensions.json` and offline blocklist cache at `extensions/registry/blocklist.json`. |
