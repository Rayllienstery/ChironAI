...
_OPERATION_DETAILS: dict[tuple[str, str], dict[str, str]] = {
    ...
    ("/api/webui/docker/engine/start", "POST"): {
        "summary": "Start Docker Engine",
        "description": "Starts Docker Desktop on Windows when the engine is not ready and returns immediately so CoreUI can poll until it is up.",
    },
    ...
    ("/api/webui/performance/snapshot", "GET"): {
        "summary": "Get live performance snapshot",
        "description": "Returns system RAM, ChironAI host and managed Docker RAM, GPU telemetry, and the process/container rows for Performance → Details.",
    },
    ("/api/webui/host/phone-status", "GET"): {
        "summary": "Get compact phone-host status",
        "description": "Returns whether ChironAI is up and whether generation is in flight (live LLM traces or GPU above threshold). Loopback is always allowed. Off-loopback clients must send CHIRONAI_PHONE_STATUS_TOKEN via X-Chiron-Phone-Token or Authorization: Bearer.",
    },
    ("/api/webui/host/phone-scripts", "GET"): {
        "summary": "List phone-host scripts",
        "description": "Returns the iPhone phone-host scripts and whether each is enabled for Shortcuts/SSH.",
    },
    ("/api/webui/host/phone-scripts/{script_id}", "PUT"): {
        "summary": "Enable or disable a phone-host script",
        "description": "Persists the enabled flag for one phone-host script. Disabled scripts refuse to run from SSH.",
    },
    ...
    ("/api/webui/dashboard-metrics", "GET"): {
        "summary": "Get dashboard metrics",
        "description": "Returns GPU, system CPU, system RAM, and ChironAI RAM (host process tree, Hermes, and managed Docker containers) for the CoreUI header pills.",
    },
...
def _basic_responses(path: str, method: str) -> dict[str, Any]:
    ...
    exact: dict[tuple[str, str], tuple[str, str]] = {
        ...
        ("/api/webui/docker/engine/start", "POST"): ("DockerEngineStartResponse", "Docker engine start result."),
        ...
        ("/api/webui/performance/snapshot", "GET"): ("PerformanceSnapshotResponse", "Live RAM, GPU, and process telemetry."),
        ("/api/webui/host/phone-status", "GET"): ("PhoneHostStatusResponse", "Compact host and generation status for iPhone Shortcuts."),
        ("/api/webui/host/phone-scripts", "GET"): ("PhoneHostScriptsResponse", "Phone-host scripts and enabled flags."),
        ("/api/webui/host/phone-scripts/{script_id}", "PUT"): ("PhoneHostScriptsResponse", "Updated phone-host script list."),
        ("/api/webui/rag/status", "GET"): ("RagStatusResponse", "RAG service status."),
        ("/api/webui/dashboard-metrics", "GET"): ("DashboardMetricsResponse", "GPU, CPU, and RAM telemetry for the CoreUI header."),
...
def _request_body_for(path: str, method: str) -> dict[str, Any] | None:
    ...
    exact: dict[tuple[str, str], str] = {
        ...
        ("/api/webui/host/phone-scripts/{script_id}", "PUT"): "#/components/schemas/PhoneHostScriptEnabledBody",
    ...
    return {
        "schemas": {
            ...
            "DashboardMetricsRam": {
                "type": "object",
                "properties": {
                    "system_used_bytes": {"type": "integer"},
                    "system_total_bytes": {"type": "integer"},
                    "system_used_gb": {"type": "number"},
                    "system_total_gb": {"type": "number"},
                    "app_host_bytes": {"type": "integer"},
                    "app_containers_bytes": {"type": "integer"},
                    "app_bytes": {"type": "integer"},
                    "app_gb": {"type": "number"},
                },
            },
            "DashboardMetricsCpu": {
                "type": "object",
                "properties": {
                    "utilization_pct": {"type": "number", "nullable": True},
                },
            },
            "DashboardMetricsResponse": {
                ...
                "properties": {
                    ...
                    "gpu": {"type": "object", "additionalProperties": True, "nullable": True},
                    "ram": {"$ref": "#/components/schemas/DashboardMetricsRam"},
                    "cpu": {"$ref": "#/components/schemas/DashboardMetricsCpu"},
                    "proxy_status": {"type": "string"},
                    "latest_request_seconds": {"type": "number", "nullable": True},
                    "latest_request_total_tokens": {"type": "integer", "nullable": True},
                    "latest_request_rag_steps": {"type": "object", "additionalProperties": True, "nullable": True},
                },
            },
            "ModelsListResponse": {
                "type": "object",
            ...
            "DockerEngineStartResponse": {
                ...
                "properties": {
                    ...
                    "message": {"type": "string"},
                    "error": {"type": "string"},
                    "status": {"$ref": "#/components/schemas/DockerStatusResponse"},
                },
                "additionalProperties": True,
            },
            "DockerListResponse": {
            ...
            "PerformanceMemorySlice": {
                ...
                "properties": {
                    ...
                    "committed_total_bytes": {"type": "integer"},
                    "committed_gb": {"type": "number"},
                    "committed_total_gb": {"type": "number"},
            ...
            "PerformanceAppSlice": {
                "type": "object",
                "properties": {
                    "host_bytes": {"type": "integer"},
                    "containers_bytes": {"type": "integer"},
                    "bytes": {"type": "integer"},
                    "host_gb": {"type": "number"},
                    "containers_gb": {"type": "number"},
                    "gb": {"type": "number"},
                    "host_mb": {"type": "number"},
                    "containers_mb": {"type": "number"},
                },
            },
            "PerformanceGpuSlice": {
                "type": "object",
            ...
            "PerformanceSnapshotResponse": {
                ...
                "properties": {
                    ...
                    "memory": {"$ref": "#/components/schemas/PerformanceMemorySlice"},
                    "app": {"$ref": "#/components/schemas/PerformanceAppSlice"},
                    "gpu": {"$ref": "#/components/schemas/PerformanceGpuSlice"},
                    "processes": {
                        "type": "array",
                        "items": {"$ref": "#/components/schemas/PerformanceProcessRow"},
                    },
                },
            },
            "PhoneHostStatusResponse": {
                "type": "object",
                "properties": {
                    "host": {"type": "string", "enum": ["awake"]},
                    "chiron": {"type": "string", "enum": ["up"]},
                    "generating": {"type": "boolean"},
                    "kind": {"type": "string", "enum": ["llm", "gpu"], "nullable": True},
                    "detail": {"type": "string", "nullable": True},
                    "gpu_pct": {"type": "integer", "nullable": True},
                    "active_traces": {"type": "integer"},
                    "status": {"type": "string"},
                    "message": {"type": "string"},
                },
            },
            "PhoneHostScript": {
                ...
                "properties": {
                    "id": {"type": "string"},
                    "file": {"type": "string"},
                    "wrapper": {"type": "string", "nullable": True},
                    "path": {"type": "string"},
                    "title": {"type": "string"},
                    "description": {"type": "string"},
                    "enabled": {"type": "boolean"},
                    "exists": {"type": "boolean"},
                    "wrapper_exists": {"type": "boolean"},
                },
            },
            "PhoneHostScriptsResponse": {
                ...
                "properties": {
                    "scripts": {
                        "type": "array",
                        "items": {"$ref": "#/components/schemas/PhoneHostScript"},
                    },
                },
            },
