...
- **Run full WebUI**: from the repository root, use `start_webui.bat` or `python -m webui_backend.rag_proxy` (both call `webui_backend.app_factory.create_production_app`).
...
- `webui_backend/app_factory.py`, `app.py`, `rag_proxy.py`, `paths.py` - canonical runtime entrypoints and path helpers
