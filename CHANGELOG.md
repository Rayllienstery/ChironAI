...
### Added
- RELEASE.md sections for 0.8.63 (last STABLE) and 0.9.0 PRE-RELEASE.
- README and SECURITY.md version guidance for 0.9.x prerelease vs 0.8.x STABLE.
- `.tmp_artifacts/` gitignore + root layout guard classification.
...
### Changed
- Document `v0.8.57` module-path mismatch in `docs/mutation-baseline-score.txt`.
...
### Fixed
- Made builtin extension bootstrap coverage deterministic on Windows CI while keeping sandbox behavior covered by sandbox-specific tests.

## [0.7.60] - 2026-06-23
### Fixed
- Relaxed sandbox provider descriptor timeout to stabilize Windows CI and slower beta startup environments.

## [0.7.59] - 2026-06-23
### Fixed
- Moved beta tag release validation to Linux so Docker image checks run against the target container platform.

## [0.7.58] - 2026-06-23
### Fixed
- Restored host HTTP route modules in the release tree and refreshed beta CI release checks.
...
## [0.7.1] - 2026-06-15
### Added
- Phase 0–6 foundations: quality gates, settings resolver, CoreUI lint/test harness, test splits, Docker scaffold, and shared Localization catalog.
...
### Added
- Added a repo-wide quality audit plan for staged cleanup, documentation sync, and guardrail work.

### Changed
- Synced architecture docs (`docs/ARCHITECTURE.md`, `docs/legacy_map.md`) with canonical `rag_service.*` ownership after root RAG shim removal.
