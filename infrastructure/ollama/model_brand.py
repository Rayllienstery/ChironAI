...
def get_ollama_model_brand_key(full_id: Optional[str]) -> Optional[str]:
    if not full_id:
        return None
    s = full_id.strip()
    if not s:
...
def extract_family_from_show_payload(payload: Any) -> Optional[str]:
...
def get_ollama_model_brand_key_from_family(family: Optional[str]) -> Optional[str]:
    if not family:
        return None
    s = family.strip()
    if not s:
...
def resolve_brand_key(model_id: str, show_payload: Optional[Dict[str, Any]] = None) -> Optional[str]:
    brand_key = get_ollama_model_brand_key(model_id)
    if brand_key:
        return brand_key

    if show_payload:
        family = extract_family_from_show_payload(show_payload)
        if family:
            return get_ollama_model_brand_key_from_family(family)
