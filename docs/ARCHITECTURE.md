...
- **`CoreModules/ClawCode`**: optional distribution `clawcode`; separate-thread **OpenAI- and Anthropic-compatible** **agent** HTTP on port 8082 (`POST /v1/chat/completions` and `POST /v1/messages`, tool `rag_query` → existing RAG), optional MCP **info** HTTP on 8083, WebUI API under `/api/webui/clawcode`. Removing this tree + wiring hooks leaves the rest of ChironAI intact. See [`Claw.md`](../Claw.md) and [`CoreModules/ClawCode/README.md`](../CoreModules/ClawCode/README.md).
